...
for subject_score in scores.values():
    ...
    #total_score += subject_score (위랑 이거랑 같은 코드)
average_score = total_score / len(scores) #length (scores의 갯수를 뜻함)
print(average_score)
...
# person_score 는 a를 의미 individual score는 b를 의미 
# a의 value가 딕셔너리
# 아래에 코드를 작성해 주세요.
print('==== Q2 ====')
total_score = 0
...
for name, temp in city.items():
    ...
    if count == 0:
        ...
        if max(temp) > hot_temp:
            ...
            hot_city = name
    count += 1
print(f'최고로 더웠던 지역은{hot_city}로 {hot_temp}도 였고, 최고로 추웠던 지역은{cold_city}로 {cold_temp}도 였다.')
