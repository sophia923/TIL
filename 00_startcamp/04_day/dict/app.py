...
def lotto_result():
    ...
    # for numb in my_numbers:
        # numbers.append(int(num)) 위랑 아래랑 같음
    ...
    matched = 0
    # 내 번호 리스트를 돌면서 / 뽑은 번호 하나 하나가 각각 winner 리스트에 있는지 확인 있으면 +1 없으면 뭐..
    for num in numbers:
        if num in winner:
            matched += 1
    # 나 개쩔 코드 2 
    # matched = len(set(winner) & set(numbers)) 위에 for numb in numbers:  부터 matched += 1 까지 쓴 걸 하나에 씀 
    if len(numbers) == 6:
        if matched == 6:
            result = '1등입니다!'
        elif matched == 5:
